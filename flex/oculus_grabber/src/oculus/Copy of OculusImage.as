	...
	public class OculusImage 
	{
		...
		public function findBlobStart(x:int, y:int, w:int, h:int, bar:ByteArray):Array { // calibrate only...
			...
			if (blob.length>1) { // error, its always greater than 1!!  should be 'contains at least one TRUE'? or just remove condition
				...
				lastTopRatio = getPixelEqTrueCount(blob, minx, maxx, miny, miny+(maxy-miny)*0.333) / blobSize/3;
				lastBottomRatio = getPixelEqTrueCount(blob, minx, maxx, miny+(maxy-miny)*0.666, maxy) / blobSize/3;
				lastMidRatio = getPixelEqTrueCount(blob,minx,maxx,miny+(maxy-miny)*0.333, miny+(maxy-miny)*0.666) / blobSize/3;
			...
			for (pixel=0; pixel<=width*height; pixel++) {
				if (parr[pixel]) { // finds a white one
					...
					if (blobSize > 150) { // discard tiny blobs
						...
						miny = r[2];
						maxy = r[3];  
						topRatio = getPixelEqTrueCount(blobs[blobnum], minx, maxx, miny, miny+(maxy-miny)*0.333) / blobSize/3;
						bottomRatio = getPixelEqTrueCount(blobs[blobnum], minx, maxx, miny+(maxy-miny)*0.666, maxy) / blobSize/3;
						midRatio = getPixelEqTrueCount(blobs[blobnum],minx,maxx,miny+(maxy-miny)*0.333, miny+(maxy-miny)*0.666) / blobSize/3;
						blobRatio = (maxx-minx)/(maxy-miny);
						diff = 0;
						rn = Math.abs(topRatio - lastTopRatio)/lastTopRatio;
						if (rn > 0.25) { rn = 99 };
						diff += rn;
						rn = Math.abs(bottomRatio - lastBottomRatio)/lastBottomRatio;
						if (rn > 0.25) { rn = 99 };
						diff += rn;
						rn = Math.abs(midRatio - lastMidRatio)/lastMidRatio;
						if (rn > 0.25) { rn = 99 };
						diff += rn;
						//diff = Math.abs(topRatio - lastTopRatio) + Math.abs(bottomRatio- lastBottomRatio) + Math.abs(midRatio- lastMidRatio);
						if (diff < maxdiff && blobRatio < lastBlobRatio * 1.1 && blobRatio > lastBlobRatio * 0.6) {
							winner=blobnum;
							// winnerBlobSize = r[4];
							maxdiff = diff;
							winRect = r.slice();
